...
SABIAN_DATA = {
    ...
    "射手座": ["軍隊のキャンプ", "白波に覆われた大洋", "チェスのプレーヤー", "子供と歩く小さな犬", "古いフクロウ", "クリケットの試合", "ドアを叩くキューピッド", "岩の中の住居", "母親に連れられる子供", "黄金の翼を持つ鳥", "左側の意志の力の影", "旗を振る情景", "未亡人の過去", "ピラミッドとスフィンクス", "自分の影を探すグラウンドホッグ", "船を見守るカモメ", "日の出の礼拝", "帽子をかぶった子供", "ペリカンの家", "氷の上で遊ぶ子供たち", "借りた眼鏡をかける子供", "中国の洗濯屋", "移民が入国する", "家の玄関の青い鳥", "おもちゃの馬に乗る少年", "旗手", "彫刻家", "橋を渡る馬車", "芝生を刈る男", "法王"],
...
place_name = st.sidebar.selectbox("生まれた場所", list(CITY_DB.keys()), index=12) # 東京
...
def draw():
    ...
    for i in range(12):
        ...
        ax.text(theta + np.deg2rad(15), 1.3, z_jp[i], ha='center', va='center', fontsize=9)
    ...
    if t_mode in ["ネイタル", "重ね表示"]:
        for i, c in enumerate(cusps):
            ...
            mid = np.deg2rad(c + 15) # 簡易表示
            ax.text(mid, 0.4, str(i+1), color="gray")
    ...
    if t_mode in ["ネイタル", "重ね表示"]:
        for k, v in n_pos.items():
            ...
            ax.text(theta, 0.6, v['name_jp'], ha='center')

    if t_mode in ["重ね表示", "トランジットのみ"]:
        now = datetime.datetime.now()
        t_pos, _ =import streamlit as st
import swisseph as swe
import matplotlib.pyplot as plt
import numpy as np
import datetime
...
def load_font():
    ...
    if not os.path.exists(font_path):
        try:
            ...
            pass # 失敗した場合は標準フォントで続行
    return font_path

# この設定で日本語を表示できるようにします
font_path = load_font()
...
for i in range(12):
    theta = np.deg2rad(i * 30)
    ax.plot([theta, theta], [0, 1.2], color="#ddd", lw=1)
    ax.text(theta + np.deg2rad(15), 1.3, z_jp[i], ha='center', va='center')
...
if t_mode in ["重ね表示", "トランジットのみ"]:
    ...
    for k, v in t_pos.items():
        ...
        ax.text(theta, r+0.1, v['name_jp'], color='red', ha='center')
...
for k, v in target_pos.items():
    ...
    sign = z_jp[s_idx]
    st.write(f"**{v['name_jp']}**: {sign} {s_deg}度 『{SABIAN_DATA[sign][s_deg-1]}』") calc_pos(now.year, now.month, now.day, now.hour, now.minute)
        for k, v in t_pos.items():
            theta = np.deg2rad(v['deg'])
            r = 0.7 if t_mode == "トランジットのみ" else 0.9
            ax.plot(theta, r, 'ro', markerfacecolor='white')
